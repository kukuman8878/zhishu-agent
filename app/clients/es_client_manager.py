...
class ESClientManager:
    ...
    # 构造函数：保存 ES 配置并预留 client 字段；参数 es_config=含 host/port 的 Elasticsearch 配置对象
    ...
    # 根据配置拼接 Elasticsearch 服务地址并返回；参数：无(使用 self.es_config)
    def _get_url(self) -> str:
        """拼接 Elasticsearch 服务地址"""
        return f"http://{self.es_config.host}:{self.es_config.port}"

    # 创建并初始化异步 Elasticsearch 客户端；参数：无
    def init(self):
        """
...
if __name__ == "__main__":
    ...
    # 本地调试：执行一次索引创建/写入与查询以验证 ES 接入链路，结束时关闭连接；参数：无
    async def test():
