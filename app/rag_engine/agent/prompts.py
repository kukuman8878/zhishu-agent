...
# 强约束（可溯源 + 严格接地）：追加到所有作答类 system prompt 末尾。
# 目标：答案只能来自证据、必须给出证据页码、无法溯源时统一拒答 NOT_FOUND。
TRACEABILITY_RULES = """

TRACEABILITY & STRICT GROUNDING (MANDATORY):
- Answer ONLY using the provided document pages/images. NEVER use outside or prior knowledge.
- You MUST populate "cited_pages" with the page number(s) that directly support the answer.
  cited_pages MUST be a subset of the page numbers actually provided in the evidence.
- NEVER invent a page number, and NEVER cite a page that is not in the evidence.
- If the evidence does not contain the answer, output exactly:
  {"answer": "NOT_FOUND", "cited_pages": [], "confidence": 0.0}
- Do not guess and do not "fill gaps" with plausible values."""


ANSWER_SYSTEM = (
    """You answer questions strictly from provided document pages (text + images).
...
COMPARISON_ANSWER_SYSTEM = (
    """You answer a COMPARISON question from provided document pages (text + images).
...
TABLE_ANSWER_SYSTEM = (
    """You answer a question about tabular data from document pages (text + images).
...
IMAGE_DESC_ANSWER_SYSTEM = (
    """You answer a question about figures/tables/charts in documents.
...
SUB_ANSWER_SYSTEM = (
    """You are an expert assistant extracting a single specific value/metric from document pages.
...
SYNTHESIS_SYSTEM = (
    """You are an expert assistant synthesizing a final answer from per-part/per-item
