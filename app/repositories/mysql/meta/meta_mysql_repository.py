...
class MetaMySQLRepository:
    ...
    # 批量把表元数据实体加入会话待提交(ORM 转换走 TableInfoMapper)；参数 table_infos=表元数据实体列表
    ...
    # 批量把字段元数据实体加入会话待提交(ORM 转换走 ColumnInfoMapper)；参数 column_infos=字段元数据实体列表
    ...
    # 批量把指标元数据实体加入会话待提交(ORM 转换走 MetricInfoMapper)；参数 metric_infos=指标元数据实体列表
    ...
    # 批量把字段-指标关联关系实体加入会话待提交(ORM 转换走 ColumnMetricMapper)；参数 column_metrics=关联关系实体列表
    ...
    # 按 id 查询单个字段元数据并转成实体返回，供召回合并阶段补齐字段；参数 id=字段主键 id
    ...
    # 批量按 id 查询多个字段元数据，返回 id→ColumnInfo 字典，解决 N+1 查询问题；参数 ids=字段主键 id 列表
    async def get_column_infos_by_ids(self, ids: list[str]) -> dict[str, ColumnInfo]:
        """批量按 id 查询字段元数据，返回 {id: ColumnInfo} 字典"""
        if not ids:
            return {}
        # expanding 绑定参数把 IN (:ids) 正确展开成多个占位符；转换仍走 Mapper，保证与单查逻辑一致
        result = await self.session.execute(
            text("SELECT * FROM column_info WHERE id IN :ids").bindparams(
                bindparam("ids", expanding=True)
            ),
            {"ids": ids},
        )
        return {
            row["id"]: ColumnInfoMapper.to_entity(ColumnInfoMySQL(**dict(row)))
            for row in result.mappings().fetchall()
        }

    # 按 id 查询单个表元数据并转成实体返回，用于组装提示词表结构；参数 id=表主键 id
    ...
    # 批量按 id 查询多个表元数据，返回 id→TableInfo 字典；参数 ids=表主键 id 列表
    async def get_table_infos_by_ids(self, ids: list[str]) -> dict[str, TableInfo]:
        """批量按 id 查询表元数据，返回 {id: TableInfo} 字典"""
        if not ids:
            return {}
        result = await self.session.execute(
            text("SELECT * FROM table_info WHERE id IN :ids").bindparams(
                bindparam("ids", expanding=True)
            ),
            {"ids": ids},
        )
        return {
            row["id"]: TableInfoMapper.to_entity(TableInfoMySQL(**dict(row)))
            for row in result.mappings().fetchall()
        }

    # 查询指定表的主外键字段实体列表，兜底补齐 Join 所需的关键字段；参数 table_id=表主键 id
