...
class ValueESRepository:
    ...
    # 构造函数：保存 ES 异步客户端；参数 client=AsyncElasticsearch 实例
    ...
    # 按关键词全文检索字段取值并保留相关分，返回 (ValueInfo, score) 列表供后续语义重排；
    # 参数 keyword=检索关键词，score_threshold=最低相关分，limit=返回条数上限(粗召回阶段可放大以便重排)
    async def search(
        self, keyword: str, score_threshold: float = 0.4, limit: int = 30
    ) -> list[tuple[ValueInfo, float]]:
        """按关键词全文检索字段取值，并还原为 ValueInfo 实体，同时返回相关分"""
        ...
        return [
            (ValueInfo(**hit["_source"]), hit["_score"]) for hit in resp["hits"]["hits"]
        ]
