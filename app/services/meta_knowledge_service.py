...
class MetaKnowledgeService:
    ...
    # 构造函数：保存 Meta/DW 仓储、字段与指标 Qdrant 仓储、ES 仓储与 Embedding 客户端等依赖；参数为各仓储与客户端对象
    def __init__(
        self,
    ...
    # 从数仓补齐字段真实类型/示例值并连同表信息落库 Meta MySQL，返回字段实体列表；参数 meta_config=元数据构建配置
    ...
    # 为字段建立向量索引：把字段名/描述/别名各自生成向量点并批量写入 Qdrant；参数 column_infos=字段业务实体列表
    ...
    # 把配置标记为同步的字段真实取值(全量)写入 ES 全文索引；参数 meta_config=构建配置，column_infos=字段实体列表
    ...
    # 把指标及其依赖字段关系(同一事务)写入 Meta MySQL，返回指标实体列表；参数 meta_config=元数据构建配置
    ...
    # 为指标建立向量索引：把指标名/描述/别名各自生成向量点并批量写入 Qdrant；参数 metric_infos=指标业务实体列表
    ...
    # 构建入口：读取配置文件后按配置依次构建 Meta MySQL / 字段与取值索引 / 指标及其向量索引；参数 config_path=meta 配置文件路径
